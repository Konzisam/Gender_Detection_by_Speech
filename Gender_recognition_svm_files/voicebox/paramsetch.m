...
%      Version: $Id: paramsetch.m 9302 2017-01-18 16:19:20Z dmb $
%
...
dn=fieldnames(d);
ndn=length(dn);
dup=zeros(length(dn),1); % update flag for existing fields
...
if nargin<5
    ...
    if nargin<3
        m='';
        if nargin>=2 && numel(q)>0
        end
    end
end
% now update the selected fields
if nargin>1 && numel(q)>0
    if isstruct(q)
        qn=fieldnames(q);
        addnew=~any(lower(m)=='a');
        adderr=any(m=='A');
        for i=1:length(qn)
            fi=qn{i};
            old=isfield(p,fi);
